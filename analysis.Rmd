...
Hey-oh now we're coookin!  According to our log-log model <b>`r r_squared`%</b> of the variance in the natural log of acres enrolled in CRP can be explained by the variance in the natural log of average number of barrels of crude produced in these same states. 
...
Furthermore, since this is a [log-log model](https://stats.idre.ucla.edu/other/mult-pkg/faq/general/faqhow-do-i-interpret-a-regression-model-when-some-variables-are-log-transformed/) we know that the slope coefficient is the [elasticity](https://stackoverflow.com/a/56614010/8623742).  Therefore, according to our log-log model, a <b>100% increase</b> in the Avg Barrel Produced per Day in 1000's will result in an approximate <b>`r slope_coeff`% `r increase_decrease_str`</b> in Acres Enrolled in CRP.